...
if (!function_exists('sng_set_post_views')) {
  function sng_set_post_views($postID) {
    ...
    add_action('wp_footer', function () use ($postID) {
      $site_url = site_url();
    ...
    <div class="widget my_profile">
      <div class="yourprofile">
        <div class="profile-background">
          <img src="<?php echo $background[0] ?>" width="<?php echo $background[1] ?>" height="<?php echo $background[2] ?>">
        </div>
...
if (!function_exists('sng_show_box_menu')) {
  function sng_show_box_menu($option) {
    ...
    <div class="widget">
      <?php if ($title) { ?>
        <div class="widget-menu__title main-c pastel-bc strong<?php if($center): echo ' ct'; endif;?>" style="background-color: <?php echo $background_color ?>; color: <?php echo $color ?>">
          <?php if ($icon) { ?>
...
/* 初期インストール時にはレガシーウィジェットを有効化する */
if (!function_exists('sng_after_switch_theme')) {
  function sng_after_switch_theme() {
    update_option('sng_use_legacy_widget', 1);
  }
}
...
add_action('after_switch_theme', 'sng_after_switch_theme');
