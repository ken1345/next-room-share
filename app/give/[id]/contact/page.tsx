...
export default function GiveContactPage() {
    ...
    const [form, setForm] = useState({
        message: '投稿を拝見しました。譲っていただけないでしょうか？'
    });
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (!session) {
                router.push(`/login?redirect=/give/${id}/contact`);
                return;
            }
            ...
            if (itemError) {
                ...
                setItem({ ...itemData, users: posterData });
            }
        ...
        return (
            <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4 font-sans">
                <div className="bg-white p-8 rounded-xl shadow-lg max-w-md w-full text-center">
                    ...
                    <Link href={`/give/${id}`} className="block w-full bg-gray-100 text-gray-800 font-bold py-3 rounded-xl hover:bg-gray-200 transition">
                        戻る
                    </Link>
                </div>
            </div>
        ...
        <div className="min-h-screen bg-gray-50 pb-20 font-sans">
            <div className="bg-white border-b sticky top-0 z-10">
                <div className="container mx-auto px-4 h-16 flex items-center">
                    <Link href={`/give/${id}`} className="text-gray-500 hover:text-gray-800 font-bold flex items-center gap-1">
                        <MdArrowBack /> 戻る
                    </Link>
            ...
            <div className="container mx-auto px-4 py-8 max-w-2xl">
                <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm mb-6">
                    <h2 className="font-bold text-gray-800 mb-2">送信先: {item.users?.display_name || 'ゲスト'}</h2>
                    <p className="text-sm text-gray-800 font-bold">{item.title}</p>
                </div>
