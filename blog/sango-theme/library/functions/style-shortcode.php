...
if (!function_exists('sng_get_entry_link_data')) {
  function sng_get_entry_link_data($id, $thumb_size, $is_date) {
...
<a class="c_linkto longc_linkto" href="{$url}"{$target}>
  ...
  <span class="longc_content c_linkto_text">{$date}<span class="longc_title">{$title}</span></span>
</a>
EOF;
      } // endif
    } // end foreach
...
<a class="sidelong__link" href="{$url}"{$target}>
  <p class="sidelong__img">
    {$img}
  </p>
  <div class="sidelong__article-info">
    {$date}
    <p class="sidelong__title">{$title}</p>
  </div>
</a>
</article>
EOF;
...
if (!function_exists('sng_get_cat_tag_post_output')) {
  function sng_get_cat_tag_post_output($array_posts, $type, $is_date, $infeed = false) {
    ...
    switch ($type) {
      case "card":
        ...
        foreach ($array_posts as $i => $post) {
          ...
          $output .= sng_card_link(array('id' => $post->ID, 'is_date' => $is_date));
        }
      ...
      case "card3":
        ...
        foreach ($array_posts as $i => $post) {
          ...
          $output .= sng_longcard_link_half(array('id' => $post->ID));
        }
        $output = '<div class="sidelong sidelong--shade cf">'.$output.'</div>';
        break;
      // sng_sidelong_card
      default:
        foreach ($array_posts as $post) {
          $output .= sng_entry_link(array('id' => $post->ID, 'is_date' => $is_date));
        }
    }
    return $output;
  }
