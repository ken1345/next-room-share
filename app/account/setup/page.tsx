        ...
        try {
            ...
            const { error } = await supabase
                ...
                .upsert({
                    ...
                    // Preserve existing photo if any? We assume trigger handles it or we don't touch it here.
                    // Ideally we should select existing first, but upsert merges if we don't specify all columns? 
                    // No, upsert replaces unless we handle conflict carefully or select first.
                    // For simplicity, we just assume display_name/gender/age/occupation are what we care about.
                    // Actually upsert might overwrite photo_url if we don't include it.
                    // Let's use UPDATE if row exists, but we don't know for sure.
                    // Safer to update only specific columns using .update().
                })
            // Wait, if row doesn't exist (trigger failed), update fails.
            ...
            router.push('/account'); // Or home, or return url
