<?php
  ...
  */
  ...
  //画像のみ表示
?>
  <div class="<?php if (get_option('limit_header_width')){echo 'maximg center';} else {echo 'center';} ?>">
    <?php 
...
<?php else : //ボタン・テキストも表示する場合?>
  <div id="header-image" class="header-image <?php if (get_option('limit_header_width')) echo 'maximg'; ?>" style="background-image: url(<?php echo esc_url(get_option('original_image_upload')); ?>);">
  ...
  <div id="divheader" class="divheader maximg" style="background: <?php echo get_theme_mod( 'divide_background_color', '#93d1f0'); ?>;">
    ...
    <div class="divheader__text">
      ...
      <?php if(get_option('divheader_btn_txt')): ?>
        <p class="divheader__btn"><a class="raised rippler rippler-default" href="<?php echo esc_url(get_option('divheader_btn_url')); ?>" style="background: <?php echo get_theme_mod( 'divide_btn_color', '#6BB6FF'); ?>;"><?php echo get_option('divheader_btn_txt'); ?></a></p>
      <?php endif; ?>
    </div>
  </div>
<?php endif; ?>
